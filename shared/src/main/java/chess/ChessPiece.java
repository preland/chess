...
public class ChessPiece {
    ...
    public boolean dothing2 (int i, ChessBoard board, ChessPosition myPosition, int nv, int sv, int ev, int wv, boolean nb, boolean sb, boolean eb, boolean wb, boolean nbt, boolean sbt, boolean ebt, boolean wbt) {
        if(!nb) {
            nbt = nv>2;
        }
        if(!sb) {
    ...
    public Collection<ChessMove> handleKingMoves(ChessBoard board, ChessPosition myPosition, boolean checkcheck) {
        ...
        for (int i = 1; i < 2; i++) {
            ...
            dothing2(i, board, myPosition, nv, sv, ev, wv, nb, sb, eb, wb, nbt, sbt, ebt, wbt);
    ...
    public boolean dothing1 (int i, ChessBoard board, ChessPosition myPosition, boolean nb, boolean sb, boolean eb, boolean wb,
    ...
    public Collection<ChessMove> handleQueenMoves(ChessBoard board, ChessPosition myPosition, boolean checkcheck) {
        ...
        boolean wbt = false;
        for (int i = 1; i < 9; i++) {
            if(!dothing1(i, board, myPosition, nb, sb, eb, wb, nbt, sbt, ebt, wbt, checkcheck, validMoves)) {
                break;
            }
        }
        nb = false;
    ...
    public Collection<ChessMove> handlePawnMoves(ChessBoard board, ChessPosition myPosition, boolean checkcheck) {
        ...
        for (int i = 1; i < 2; i++) {
            ...
            if(nv==1 && !nb && board.getPiece(myPosition).getTeamColor()== ChessGame.TeamColor.WHITE) {
              //separate all in if
                ChessHelper.help1(i, board, myPosition, validMoves, checkcheck);

            }

            if(sv==1 && !sb && board.getPiece(myPosition).getTeamColor()== ChessGame.TeamColor.BLACK) {
              //separate all in if
                ChessHelper.help2(i, board, myPosition, validMoves, checkcheck);
            }

            if(ev==3 && !eb && i<2) {
              //separate all in if
                ChessHelper.help3(i, board, myPosition, validMoves, checkcheck);
            }

            if(wv==3 && !wb && i<2) {
            //separate all in if
                ChessHelper.help4(i, board, myPosition, validMoves, checkcheck);
            }
            nb = nbt;
            sb = sbt;
            eb = ebt;
